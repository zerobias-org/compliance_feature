# Compliance feature mono-repo
Monorepo of compliance feature artifacts.

# Starting Development
##  Fork Repository

Under the [zerobias-org/compliance_feature](https://github.com/zerobias-org/compliance_feature) repo, click the Fork button toward top right of the screen
* Under owner choose your personal account for development, and leave the repository name as compliance feature
...
* Any PRs made against the `zerobias-org/compliance_feature` repo should be agaisnt it's dev branch or they will be rejected.
...
### Create new compliance feature folder

Create a folder for your new compliance feature with the following naming convention:
f\_{nameFirstLetters}\_{possibleIncrement} (ex. c_bad)
...
* nameFirstLetters: first letter of each word in the compliance feature name made lowercase: ex. bad (Business Application Development)
...
### Run create new compliance feature script

Run the folowing script `sh scripts/createNewCompliancefeature.sh <folder_path>`
...
* You will need to edit the index.yml file anywhere you see a {variable}
  * {name}: Replace this with the name of your compliance feature
  * {description}: Replace this with the description of your compliance feature
  * {externalId}: Replace this with a external view name of the compliance feature, can just use name
  * Add aliases if you would like any aliases set for this compliance feature
...
Run the following commands to update npm for your new compliance feature
* `cd <folder_path>` cd into your new compliance feature directory
* `npm install` run npm install
* `npm shrinkwrap` run npm shrinkwrap

### Validate new compliance features

In the root of the respository run the follow command to validate all edit or added compliance features
...
#### Compliance feature versioning
Compliance feature versions are managed by `lerna`. 
* Add it to [lerna.json](./lerna.json) to enable lerna for a compliance feature.
* The starting version of a compliance feature should be `0.0.0`
* There should be no version bumps inside of pull requests.

Lerna will automatically version, generate changelogs and publish compliance features in `lerna.json` via our github actions workflows.
...
In order for a compliance_feature/package to bump to `1.0.0` it must be explicitely told to do so via `premajor` and `conventional graduation`.
...
Each compliance feature should have one workflow that runs tests on Pull Request.
...
#### Lerna Publish
This workflow will run lerna publish against all registered compliance features.
Compliance features that have changed are first bootstraped then tagged and published.
...
#### Lerna Post Publish
This workflow is used to finalize compliance feature publication. At the moment it:
...
#### Build compliance feature affected by change

The affected compliance feature(s) will be built via `npm run build` and `unit` tested.
